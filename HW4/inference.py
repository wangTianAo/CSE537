...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            allPossible[ghostPosition] = 1.0
            for position in allPositions:
                allPossible[position] = 0
        else:
            for position in allPositions:
                mhdistance = util.manhattanDistance(pacmanPosition, position)
                if emissionModel[mhdistance] > 0:
                    allPossible[position] = emissionModel[mhdistance]*self.beliefs[position]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in allPositions:
            ...
            for newpd in newPosDist.items():
                newpPosition = newpd[0]
                probability = newpd[1]
                allPossible[newpPosition] += probability*self.beliefs[position]
        allPossible.normalize()
        self.beliefs=allPossible
